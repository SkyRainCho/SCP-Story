...
用于将 SCP Wiki CN 的 Tales Edition 目录页下载、清洗并打包为 EPUB 电子书的 Python 工具。当前仓库已支持按配置文件生成 Series 1 到 Series 8 的分卷 EPUB，也支持从英文站 Featured SCP Archive 生成中文精选主文档 EPUB；构建过程中会缓存页面、下载正文图片等资源、生成中间 XHTML 与构建报告。正文清洗为 CPU 密集型，页面与附属文档抓取为 I/O 密集型，二者均已并行化以加速大规模构建。
...
设施卡片图标最大为 192×192；“兴趣地点”徽标最大为 320×320，其地图和其他非徽标
...
- **正文清洗（CPU 密集）** 用多进程并行，默认最多 8 个进程。可用环境变量
  `SCP_EPUB_WORKERS` 调整：设为 `1` 走纯串行（便于比对排错），设为更大值在多核机器上
  进一步加速。每个进程池子任务内部读取页面文件，避免大段正文跨进程序列化。
- **页面与附属文档抓取（I/O 密集）** 用多线程并行，默认 6 线程。可用环境变量
  `SCP_EPUB_FETCH_WORKERS` 调整；缓存全部命中的分卷（如已抓全的 Series 1–3 和精选集）
  几乎没有网络开销，并行与串行差异不大，主要加速未抓全的分卷。

此外，对永久性 4xx（404 / 403 / 410 等）页面不再重试，仅在 5xx、网络错误和瞬时性
408 / 429 上重试。后期 Series（如 Series 7、8）有大量尚未翻译、返回 404 的 SCP 编号，
跳过重试可显著减少这类分卷的构建时间。

> 并发抓取会提高对站点的瞬时请求量。若遇到限流，可适当调小
> `SCP_EPUB_FETCH_WORKERS`，或在配置中保留合理的 `request_delay_seconds`。

在缓存就绪的实测中，`SCP基金会档案精选` 的构建由约 531 秒降至约 65 秒；未抓全的后期分卷
单册可由数百秒降至约 1 分钟量级。
...
- `request_delay_seconds`、`request_timeout_seconds`、`retry_count`：页面请求节流、超时与重试设置。永久性 4xx 错误（404 / 403 / 410 等，不含 408 / 429）不会重试。
...
5. 下载或读取缓存中的页面 HTML；多个页面并行抓取。
6. 清洗正文，标准化资源 URL 与链接；正文清洗跨进程并行。
7. 下载正文资源并写入 EPUB。
...
- 页面抓取默认多线程并发；如需顺序确定的比对，可置 `SCP_EPUB_FETCH_WORKERS=1`。测试通过 `tests/conftest.py` 固定该变量为 `1` 以保证依赖调用顺序的断言稳定。
